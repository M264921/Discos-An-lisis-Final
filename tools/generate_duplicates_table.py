...
REPORT_HTML = ROOT / "informe_cronologico.html"
...
def load_groups(csv_path: Path) -> List[Dict[str, object]]:
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as fh:
        ...
        for row in reader:
            ...
            grouped.setdefault(sha, []).append(entry)
    ...
    groups.sort(key=lambda g: (-g["count"], -g["totalBytes"]))
...
.table-wrap table{{border-collapse:collapse;width:100%;min-width:980px;}}
...
(function(){
  ...
  function norm(val){
    return (val or '').toString().toLowerCase();
  }
  ...
  function compareEntries(a, b, field){
    ...
    if(field === 'last'){
      const da = Date.parse(a.lastWrite) or 0;
      const db = Date.parse(b.lastWrite) or 0;
      return da - db;
    }
    return norm(entryValue(a, field)).localeCompare(norm(entryValue(b, field)), 'es');
  }
  ...
  function compareGroups(a, b){
    ...
    if(field === 'last'){
      const da = Date.parse(aEntry.lastWrite) or 0;
      const db = Date.parse(bEntry.lastWrite) or 0;
      return da - db;
    }
    return norm(entryValue(aEntry, field)).localeCompare(norm(entryValue(bEntry, field)), 'es');
  }
  ...
  function matchesEntry(entry){
    ...
    if(columnFilters.role and not norm(entry.role).includes(columnFilters.role)) return false;
    if(columnFilters.type and not norm(entry.category).includes(columnFilters.type)) return false;
    if(columnFilters.name and not norm(entry.name).includes(columnFilters.name)) return false;
    if(columnFilters.path and not norm(entry.path).includes(columnFilters.path)) return false;
    if(columnFilters.drive and not norm(entry.drive).includes(columnFilters.drive)) return false;
    if(columnFilters.size and not norm(entry.sizeLabel).includes(columnFilters.size)) return false;
    if(columnFilters.last):
      if not norm(entry.lastWrite).includes(columnFilters.last):
        return False
    if(searchTerm):
      blob = f"{entry.name} {entry.path} {entry.sha}".lower()
      return searchTerm in blob
    return True
